  ...
  return new Promise((resolve, reject) => {
    ...
    const request = https.request({
      ...
      response.on('end', () => {
        ...
        else reject(Object.assign(new Error(parsed.error?.description || `Payment gateway error (${response.statusCode}).`), { statusCode: response.statusCode }));
      });
    });
...
async function createRazorpayOrder(payload) {
  if (razorpayClient) return razorpayClient.orders.create(payload);
  return razorpayRequest('POST', '/v1/orders', payload);
}
...
router.post('/:id/payment-order', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const amount = Math.round(Number(booking.grandTotal || booking.totalAmount) * 100);
    if (!Number.isInteger(amount) || amount < 100) {
      return res.status(400).json({ message: 'Payment amount must be at least 100 paise.' });
    }
    ...
    const status = Number(error.statusCode || error.status) === 401 ? 401 : 500;
    return res.status(status).json({
      message: status === 401
        ? 'Razorpay authentication failed. Check the server credentials.'
        : (error.error?.description || error.message || 'Payment gateway could not create an order.')
    });
  }
});
...
router.post('/:id/cancel', requireAuth, async (req, res) => {
  ...
  if (earnedBeforeCancel) {
    ...
    await notifyUser(booking.ownerId, { type: 'booking', title: 'Booking cancelled', message: `A confirmed booking for your vehicle was cancelled, so its earnings were reversed.`, data: { bookingId: booking._id.toString() } });
  } else {
