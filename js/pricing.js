...
<<<<<<< HEAD
  // Must mirror backend/utils/pricing.js exactly: sub-minute timestamp drift is
  // absorbed so an exact 48-hour booking bills 2 days, not 3.
  const hours = Math.max(1, Math.ceil(((end - start) - 60 * 1000) / 3600000)); const unit = clientPriceUnit(vehicle.priceUnit); const price = Math.max(0, clientNumber(vehicle.price));
=======
...
<<<<<<< HEAD
  const discountPercent = Math.max(0, Math.min(100, clientNumber(vehicle.discountPercent))); const taxPercent = Math.max(0, Math.min(100, clientNumber(vehicle.taxPercent)));
  const subtotal = clientRoundMoney(baseRentalAmount + extraKilometerCharges + additionalCharges); const discountAmount = clientRoundMoney(subtotal * discountPercent / 100); const discountedSubtotal = clientRoundMoney(Math.max(0, subtotal - discountAmount));
  const taxFees = clientRoundMoney(discountedSubtotal * taxPercent / 100); const grandTotal = clientRoundMoney(discountedSubtotal + taxFees);
  return { price: clientRoundMoney(price), priceUnit: unit, currency: 'INR', durationHours: hours, durationLabel: `${hours} hour(s)`, billableUnits, baseRentalAmount, includedKm, estimatedKm: clientRoundMoney(km), extraKm, extraKmRate: clientRoundMoney(extraKmRate), extraKilometerCharges, additionalCharges, discountPercent, discountAmount, discountedSubtotal, taxPercent, taxFees, subtotal, grandTotal, paidAmount: 0, remainingAmount: grandTotal, pricingVersion: 'revex-pricing-v3' };
=======
...
<<<<<<< HEAD
    <div class="quote-row"><span>Discount <small>${quote.discountPercent || 0}%</small></span><strong>- ${formatMoney(quote.discountAmount || 0)}</strong></div>
=======
>>>>>>> 509eae71e1063d5f8e8f372ee9e73ca177e5dcc1
