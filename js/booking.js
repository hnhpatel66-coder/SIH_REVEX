...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (focusBooking) {
    const match = bookingItems.find(item => item.id === focusBooking);
    if (match) {
      activeBookingStatus = 'all';
      document.querySelectorAll('.tab').forEach(tab => tab.classList.toggle('active', tab.dataset.status === 'all') || (tab.dataset.status !== 'all' && tab.classList.remove('active')));
      renderBookingList();
